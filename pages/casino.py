...
def render(ctx) -> None:
    ...
    actual=m.payout/m.bets if m.bets else None
    kpis([("Observed Casino GGR",money(m.ggr),period_delta(m.ggr,previous.ggr)),("Observed Total Bets",money(m.bets),period_delta(m.bets,previous.bets)),("Observed GGR Margin",pct(m.ggr/m.bets if m.bets else None),"Observed GGR / bets"),("Observed Actual RTP",pct(actual,2),"Observed payouts / bets"),("Observed Active Games",f"{int(m.games):,}","Games with activity")],ctx)
