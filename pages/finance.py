...
def render(ctx)->None:
    ...
    kpis([("Estimated NGR",money(ngr),period_delta(ngr,previous_ngr)),("Deposits",money(tx.deposits),period_delta(tx.deposits,previous_tx.deposits)),("Withdrawals",money(tx.withdrawals),period_delta(tx.withdrawals,previous_tx.withdrawals)),("Net Cash Flow",money(tx.deposits-tx.withdrawals),"Approved deposits − withdrawals"),("Payment Approval Rate",pct(tx.approval,2),"Observed transaction approval")])
