...
def render(ctx)->None:
    ...
    m=ctx.query(f"""SELECT COALESCE(SUM(fraud_risk<.55 AND rg_risk<.55),0) targetable,COALESCE(SUM(NOT(fraud_risk<.55 AND rg_risk<.55)),0) suppressed,COALESCE(SUM(CASE WHEN fraud_risk<.55 AND rg_risk<.55 AND recommended_action IN ('Send retention campaign','Offer governed bonus','Recommend preferred game') THEN predicted_ltv_90d*.08 ELSE 0 END),0) revenue FROM v_player_scores v WHERE {active}""").iloc[0]
    ...
    with right:
        queue=ctx.query(f"""SELECT player_id,recommended_action,predicted_ltv_90d,model_confidence FROM v_player_scores v WHERE fraud_risk<.55 AND rg_risk<.55 AND recommended_action<>'Do nothing' AND {active} ORDER BY predicted_ltv_90d DESC,model_confidence DESC LIMIT 10""");st.markdown("#### Next best action");data_table(queue,column_config={"predicted_ltv_90d":st.column_config.NumberColumn("Predicted remaining LTV 90D",format="$%.0f"),"model_confidence":st.column_config.ProgressColumn("Predicted confidence",min_value=0,max_value=1,format="%.1%%")})
