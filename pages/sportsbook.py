...
def render(ctx)->None:
    ...
    kpis([("Sportsbook GGR",money(m.ggr),period_delta(m.ggr,previous.ggr)),("Handle",money(m.handle),period_delta(m.handle,previous.handle)),("Hold %",pct(m.ggr/m.handle if m.handle else 0,2),"Observed GGR / handle"),("Realized Downside",money(m.exposure),"Losses on settled bets"),("Live Bets",f"{int(m.live):,}",period_delta(m.live,previous.live))])
